//
//  AdvertisingActivityInfoDAO.swift
//  RunningMan
//
...
open class AdvertisingActivityInfoDAO: NSObject {
    
    open var delegate: AdvertisingActivityInfoDAODelegate!
    
    open static let sharedInstance: AdvertisingActivityInfoDAO = {
        let instance = AdvertisingActivityInfoDAO()
        return instance
    }()
    
    //插入Note方法
    open func create(_ model: AdvertisingActivityInfo) {
        
        let params = ["type": "JSON", "action": "add","activityImageURL":model.activityImageURL,"activityID":model.activityID]
    ...
    //删除Note方法
    open func remove(_ model: AdvertisingActivityInfo) {
    ...
    //修改Note方法
    open func modify(_ model: AdvertisingActivityInfo) {
        
         let params = ["type": "JSON", "action": "modify","activityImageURL":model.activityImageURL,"activityID":model.activityID]
                ...
                if let error = response.result.error {
                    ...
                    if resultCode >= 0 {
                        ...
                        for i in listDict {
                            ...
                            let advertisingactivityinfo=AdvertisingActivityInfo()
                            let a_id=dic["activityID"]as!NSNumber
                            advertisingactivityinfo.activityID=a_id.stringValue
                            advertisingactivityinfo.activityImageURL=dic["activityImageURL"]as!String
                            
                            listData.add(advertisingactivityinfo)
                        }
    ...
    //按照主键查询数据方法
    open func findById(_ model: AdvertisingActivityInfo) {
            ...
            .responseJSON(completionHandler: {
                ...
                if let error = response.result.error {
                    ...
                    if resultCode >= 0 {
                        ...
                        self.delegate.findByIdFinished?(advertisingactivityinfo)
